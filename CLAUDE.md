...
**No LLM on the synchronous request path.** Model work belongs to the scheduled jobs and to opt-in recall (`RETRIEVAL_ENABLE_RERANK`, `?rerank=1`, PPR's recognition filter — all default off, all degrade to a no-op). Two write paths used to break this: attachment OCR/transcription, now parked as `'pending'` for `AttachmentExtractionWorker`, and the contradiction check on `POST /facts`, now left unstamped (`supersedeCheckedAt IS NULL`) for the dreamer's supersede sweep. `INGEST_SUPERSEDE_MODE=inline` restores the old blocking behaviour for a caller that needs it. The line matters because the LLM, the embedder and the vision model share one GPU on a local deployment, so a chatty write path and the dream cycle queue behind each other — the 2026-07-11 starvation incident.
