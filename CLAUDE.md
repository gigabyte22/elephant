...
**Dreaming** (`src/services/DreamingService.ts`, the densest file). Guarded by an `AsyncMutex` (concurrent trigger → 409). Uses a persistent cursor, not "last run time", so a deadline-boxed run resumes mid-backlog; the cursor advances past *failed* episodes so a poisoned one can't pin it. Per episode: extract → embed → dedup by cosine → persist → supersede-detect. Dream-created facts set **`validFrom = episode.timestamp`** (event time) and **`recordedAt = now`** (transaction time); contradiction supersede closes the old fact at event time (`max(old.validFrom, new.validFrom)`) while `:SUPERSEDES.supersededAt` is wall clock. Then, only if the deadline held: the supersede sweep, entity resolution, consolidation, insight promotion, prune. Sub-passes are individually try/caught — facts are already committed, so a late failure must not fail the cycle.

**No LLM on the synchronous request path.** Model work belongs to the scheduled jobs and to opt-in recall (`RETRIEVAL_ENABLE_RERANK`, `?rerank=1`, PPR's recognition filter — all default off, all degrade to a no-op). Three write paths used to break this: attachment OCR/transcription, now parked as `'pending'` for `AttachmentExtractionWorker`; the contradiction check on `POST /facts`, now left unstamped (`supersedeCheckedAt IS NULL`) for the dreamer's supersede sweep; and the episode summary on `POST /episodes`, now a clipped head flagged `summaryProvisional` that the dreamer replaces and re-embeds. `INGEST_SUPERSEDE_MODE=inline` and `INGEST_DEFER_SUMMARY=false` restore the old blocking behaviour for a caller that needs it. Knowledge/research ingestion still summarizes inline — it is operator-driven, not the hot path. The line matters because the LLM, the embedder and the vision model share one GPU on a local deployment, so a chatty write path and the dream cycle queue behind each other — the 2026-07-11 starvation incident.
...
**Jobs** (`src/jobs/`). Croner schedulers, all `startXScheduler(container) => { stop, pattern }` with `protect: true`. `scripts/serve.ts` starts them and drains them on SIGTERM. Two are conditional — the OKF sweep needs `OKF_ENABLED`, the research reaper needs `RESEARCH_RETENTION_DAYS` — and both are staggered off the crons they would otherwise contend with for the driver pool (OKF 30 min off the dream cycle, the reaper 15 min off the hourly observation reap).
...
- **Nothing is hard-deleted, with one opt-in exception.** Soft-delete / prune set `validTo = now` (system forget = transaction time). The exception is research retention: with `RESEARCH_RETENTION_DAYS` set, `ResearchReaper` purges research whose `expiresAt` lapsed that many days ago, taking its chunks and its `:ArchivedRevision` snapshots (which hold the body — `serialiseForSnapshot` strips only `embedding`) and leaving the `:AuditEvent` trail. Unset by default, so an upgrade never starts deleting. Inspection by id stays ungated either way — a lapsed document is readable until the reaper takes it; recall and list are the gated surfaces. Contradiction supersede sets `validTo` to **event** time (`src/utils/temporal.ts` `eventValidTo`); consolidation merge retires members at transaction time and timeline/`snapshotAt` collapses merge members. Mutating writes to Fact/Preference/Procedure/KnowledgeDocument/Research route through `AuditService`, which snapshots an `:ArchivedRevision` and appends an `:AuditEvent` inside the caller's transaction.
...
- **Scope** has four axes (`projectId`, `userId`, `agentId`, `sessionId`) and five modes (`boost` default when a value is given, `filter`, `strict`, `shared`, `none`), applied at four layers: write (`memoryItemSetClause`), query (`scopeFilterClause` splices Cypher predicates), scoring (`scopeBoostMultiplier` + `PostFilterStage.axisAllows`), and id-addressed access (`src/http/scope-guard.ts` `assertInScope`). The subtle part is in `axisAllows`: `filter` excludes only cross-scope items since a null scope means shared/global, `strict` also excludes nulls, and `shared` keeps *only* the nulls — the one mode that carries no axis value, because a shared-space listing has no id to name. List routes take the mode per axis as a query param (`projectScope`, `userScope`, plus `agentScope`/`sessionScope` on intentions), each defaulting to `filter` when its id is supplied and `none` when it is not — so an explicit mode is the only way to ask for `shared`. `assertInScope` is the fourth layer and deliberately takes no mode: it is hard-wired to `filter`, answers a cross-scope id with 404 rather than 403 (existence is itself scoped), and treats a caller declaring *no* scope as unrestricted, which is what keeps the single-tenant default working.
