...
function toDreamRun(node: Record<string, unknown>): DreamRun {
  return {
    ...
    factsSupersedeSwept: (node.factsSupersedeSwept as number) ?? 0,
    summariesInstalled: (node.summariesInstalled as number) ?? 0,
    ...
    insightsRetired: (node.insightsRetired as number) ?? 0,
    insightsCorroborated: (node.insightsCorroborated as number) ?? 0,
  ...
  async create(tx: ManagedTransaction, run: DreamRun): Promise<DreamRun> {
    const result = await tx.run(
      `CREATE (d:DreamRun {
         ...
         factsSupersedeSwept: $factsSupersedeSwept,
         summariesInstalled: $summariesInstalled,
         factsPruned: $factsPruned,
         factsMerged: $factsMerged,
         insightsPromoted: $insightsPromoted,
         insightsRetired: $insightsRetired,
         insightsCorroborated: $insightsCorroborated,
       ...
       RETURN d {.*} AS d`,
      {
        ...
        factsSupersedeSwept: run.factsSupersedeSwept,
        summariesInstalled: run.summariesInstalled,
        factsPruned: run.factsPruned,
        factsMerged: run.factsMerged,
        insightsPromoted: run.insightsPromoted,
        insightsRetired: run.insightsRetired,
        insightsCorroborated: run.insightsCorroborated,
    ...
    const result = await tx.run(
      `MATCH (d:DreamRun {id: $id})
       SET d.completedAt = CASE
           ...
           d.factsSupersedeSwept = coalesce($factsSupersedeSwept, d.factsSupersedeSwept),
           d.summariesInstalled = coalesce($summariesInstalled, d.summariesInstalled),
           ...
           d.insightsRetired = coalesce($insightsRetired, d.insightsRetired),
           d.insightsCorroborated = coalesce($insightsCorroborated, d.insightsCorroborated),
       ...
       RETURN d {.*} AS d`,
      {
        ...
        factsSupersedeSwept: patch.factsSupersedeSwept ?? null,
        summariesInstalled: patch.summariesInstalled ?? null,
        factsPruned: patch.factsPruned ?? null,
        factsMerged: patch.factsMerged ?? null,
        insightsPromoted: patch.insightsPromoted ?? null,
        insightsRetired: patch.insightsRetired ?? null,
        insightsCorroborated: patch.insightsCorroborated ?? null,
