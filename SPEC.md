...
- **Knowledge** — `:KnowledgeDocument` + `:KnowledgeChunk` (new). Shared / RAG documents. Chunks carry two provenance properties. `derivation` is `verbatim` (the source's own words: body text, a `.txt` attachment, a PDF's text layer) or `model` (a model's rendering of non-text bytes: OCR, transcription, an image description) — it rides through to the recall payload, because "a screenshot described as showing revenue down 12%" is a different claim from "revenue is down 12%". `overlapChars` records how many leading characters were copied from the previous chunk, so a chunk set can be stitched back into its source text without repeating every seam; absent on chunks written before this existed, where 0 is the only recoverable answer.
...
Cross-cutting scope axes on every memory item: `projectId`, `userId`, plus the existing `agentId` / `sessionId`. Each axis runs in retrieval mode `boost` (default when a value is supplied), `filter` (hard match, but a null scope is a shared global and still matches), `strict` (like `filter`, and also excludes nulls), `shared` (the null-scoped globals **only**, every scoped item excluded), or `none`. `shared` is the one mode that needs no axis value, since a shared space has no id to name, and so the only one that can list such a space. Omitting the axis is a different request: that selects `none`, which spans every scope. The explicit per-axis mode params (`projectScope`, `userScope`, and on intentions `agentScope`/`sessionScope`) are accepted on the list routes — `/knowledge/documents`, `/procedures`, `/research`, `/intentions`, `/intentions/due` — as well as `/recall`; absent them each axis infers `filter` when its id is supplied and `none` when it is not. Id-addressed reads and writes take `projectId`/`userId` only and apply `filter` semantics, answering a cross-scope id with 404 rather than 403, since existence is itself scoped.
...
frontmatter (`okfVersion: 2`) when `OKF_ENABLED=true`:

```text
{OKF_DIR}/
  projects/{projectId}/research/{slug}--{id}.md
  projects/{projectId}/documents/{slug}--{id}.md  # knowledge docs w/ projectId
  projects/{projectId}/_index.md                  # generated, wikilinked
  shared/documents/{slug}--{id}.md                # knowledge docs, no scope
  shared/_index.md
  _trash/<same relative path>                     # tombstones
```

`{slug}` is the lowercased, hyphenated title, capped at 72 chars and cut at a
word boundary; a title with nothing sluggable in it yields `untitled`. The
`--{id}` suffix is mandatory and is where identity lives — titles are neither
unique nor immutable, so a slug alone would let one document overwrite
another and would move on every rename. Every live file ends in `--{id}.md`,
which is what lets the writer and the sync find a file by id with one suffix
match.
...
- **Frontmatter = identity + provenance** (id, kind, title, scope, source,
  tags, timestamps, contentHash); **body = the retained content** (summary +
  "body not retained" note for pre-retention rows). `summary` appears in the
  frontmatter **only when the body does not already open with it**, clipped
  to 1000 chars with `summaryTruncated: true` when longer. Below
  `SUMMARY_THRESHOLD_TOKENS` the ingest path stores the content verbatim as
  its own summary, so without that rule most files carried the whole document
  twice — it accounted for 30% of the vault's bytes. One consequence: for a
  pre-retention row the summary is now visible only as the body, so the file
  no longer distinguishes summary from content. Round-trip import is out of
  scope, and `contentHash: undefined` still identifies those rows.
- **Path safety.** `projectId` is an arbitrary string: segments are
  ...
  keeps collisions distinct). Titles go through a stricter slug whose output
  alphabet is `[a-z0-9-]`, so a title cannot introduce a separator, a leading
  dot, or a `_` that would collide with `_trash`/`_index.md`. Ids are
  generated by `newId()`; they are sanitized, not validated.
- **Deletion wins over portability.** Soft-delete (and lapsed expiry via
  ...
  deleted research.
- **The vault only holds files the graph still backs.** The sync reconciles
  in both directions: a file whose node is gone is moved to `_trash/` with
  `deleteReason: orphaned`, as is one left at a stale path by a title edit or
  the pre-v2 bare-id layout. Three things bound the blast radius — a **parse
  gate** (a file is only a candidate if its frontmatter carries an `id` and a
  known `kind`, so a vault directory misconfigured onto unrelated markdown is
  reported as `foreign` and left alone), an **mtime gate** (files modified
  after the sweep started belong to a concurrent request and are never
  reaped), and an **empty-graph guard** (zero projectable nodes plus a
  non-empty vault aborts the reap and logs loudly — `--force-reap` overrides).
  Nothing here hard-deletes. `pnpm okf:sync --purge` is the only delete in the
  system and is always opt-in; `--dry-run` reports the same work untouched.
- **`_index.md` is generated, never authored.** One per project bucket, listing
  that bucket's documents and research as `[[path|Title]]` wikilinks. Written
  by the sync only — building one needs a full listing, which does not belong
  on a request path — so it lags anything created since the last sweep. It
  carries no timestamp and is byte-compared before writing, so an unchanged
  project produces no write.
